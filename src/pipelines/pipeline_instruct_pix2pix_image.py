...
class StableDiffusionInstructPix2PixImagePipeline(StableDiffusionInstructPix2PixPipeline):
    ...
    def _encode_image(self, image, device, num_images_per_prompt, do_classifier_free_guidance):
        ...
        if not hasattr(self, "embedding_optimizer") or self.embedding_optimizer is None:
            ...
            # Try to locate emb_opt.pt
            possible_paths = [
                "emb_opt.pt",
                "models/emb_opt.pt",
                "src/models/emb_opt.pt",
                os.path.join(os.path.dirname(__file__), "../../models/emb_opt.pt"),
                os.path.join(os.path.dirname(__file__), "../../../emb_opt.pt")
            ]
            
            optimizer_state = None
            for path in possible_paths:
                if os.path.exists(path):
                    print(f"Loading embedding optimizer from {path}")
                    optimizer_state = torch.load(path, map_location=device)
                    break
            
            if optimizer_state is None:
                print("Warning: emb_opt.pt not found. Using random initialization for EmbeddingOptimizer.")
            else:
                for key, value in optimizer_state.items():
                    if isinstance(value, torch.Tensor):
                        optimizer_state[key] = torch.nan_to_num(value, nan=0.0)
                try:
                    self.embedding_optimizer.load_state_dict(optimizer_state)
                except RuntimeError as e:
                    print(f"Error loading state dict: {e}. Trying strict=False")
                    self.embedding_optimizer.load_state_dict(optimizer_state, strict=False)

        self.embedding_optimizer.to(device=device, dtype=dtype)
        image_embeddings = self.embedding_optimizer(image_embeddings)
